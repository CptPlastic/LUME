// LUME Firework Controller - NO WIFI TEST VERSION
// This version removes WiFi to test if that's causing the channel firing issue
...
#define FIRE_CHANNEL_3     25   // Hardware Pin 10 - THIS IS FIRING UNEXPECTEDLY
...
void setup() {
  ...
  Serial.println("=== LUME NO-WIFI TEST VERSION ===");
  Serial.println("Testing if WiFi is causing the channel 3 firing issue");
  ...
  Serial.println("LUME Firework Controller Ready (NO WIFI VERSION).");
  ...
  Serial.println("MONITORING: Watch for any unexpected channel 3 firing...");
}

void loop() {
  // NO WIFI - just handle serial commands
  ...
  if (Serial.available()) {
    ...
    if (command == "START") {
      ...
      Serial.println("Unknown command. Use: START, STOP, CHANNEL <1-12>, BUTTON <AREA_UP/AREA_DOWN/RAPID_FIRE/ALL_FIRE>, AREA <1-99>, SYNC <1-99>, STATUS");
    }
  }
